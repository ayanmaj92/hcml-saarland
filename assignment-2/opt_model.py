...
class SmartPoisson(Broadcaster):
    ...
    def get_next_interval(self, event):
        ...
        if event is None:
            ...
            self.on_top = False
            delta = self.random_state.exponential(scale=1.0 / self.rate)
            cur_time = event.cur_time
            return delta + cur_time - self.last_self_event_time
...
class Opt(Broadcaster):
    ...
    def get_next_interval(self, event):
        ...
        if event is None:
            ...
            # cur_time = self.get_current_time(event)
            delta = self.random_state.exponential(scale = 1.0)
            cur_time = event.cur_time
            
            if self.last_self_event_time + self.t_delta > cur_time + delta:
                return cur_time + delta - self.last_self_event_time
            # if ...:
            #     return t_delta
